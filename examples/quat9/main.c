 ...
 * Example6_DMP_Quat9_Orientation，改写为 Zephyr 纯 C 示例。
 */
...
int main(void)
{
	...
	/* 与 SparkFun Example6 一致，ORIENTATION 对应九轴四元数。 */
	status = inv_icm20948_enable_dmp_sensor(&icm.device,
		INV_ICM20948_SENSOR_ORIENTATION, true);
	if (status != ICM_20948_Stat_Ok) {
		LOG_ERR("启用 Quat9 失败: status=%d", status);
		return 0;
	}
	status = inv_icm20948_set_dmp_sensor_period(&icm.device,
		DMP_ODR_Reg_Quat9, 0);
	if (status != ICM_20948_Stat_Ok) {
		LOG_ERR("设置 Quat9 ODR 失败: status=%d", status);
		return 0;
	}
	...
	LOG_INF("DMP Quat9 已启动，开始轮询 FIFO");

	while (true) {
		status = inv_icm20948_read_dmp_data(&icm.device, &data);
		if ((status == ICM_20948_Stat_Ok) ||
		    (status == ICM_20948_Stat_FIFOMoreDataAvail)) {
			if ((data.header & DMP_header_bitmap_Quat9) != 0U) {
				++valid_frames;
				LOG_INF("Quat9 frame=%u header=0x%04x Q1=%" PRId32
					" Q2=%" PRId32 " Q3=%" PRId32
					" heading_accuracy=%d",
					valid_frames, data.header,
					data.Quat9.Data.Q1, data.Quat9.Data.Q2,
					data.Quat9.Data.Q3,
					(int)data.Quat9.Data.Accuracy);
			} else {
				LOG_WRN("FIFO 帧缺少 Quat9: header=0x%04x",
					data.header);
